-- DISABLED: the DAP stack was loading eagerly on every startup.
--
-- lua/config/lazy.lua sets `defaults = { lazy = false }`, and this spec has a
-- `config` function with no lazy trigger, so nvim-dap + nvim-dap-ui + nvim-nio
-- + nvim-dap-virtual-text + nvim-dap-projects (and dapui's ~20 element/component
-- modules) were all pulled in before the first screen update -- ~67ms of the
-- startup profile, for a debugger that is not in use most sessions.
--
-- To re-enable, uncomment below. Prefer adding a lazy trigger at the same time,
-- e.g. `keys = { "<F5>", "<F9>" }` or `cmd = "DapContinue"`, so it only loads
-- when debugging actually starts.
--
-- Note: plugins/debugging/adapters/gdb.lua is only reached from the config
-- function below, so it is inert while this is commented out. lazy.nvim does not
-- import it as a spec (Util.lsmod only descends into dirs with an init.lua).
